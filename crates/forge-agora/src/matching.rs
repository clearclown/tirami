...
impl CapabilityMatcher {
    ...
    pub fn find_matches(
        ...
        for agent in agents {
            ...
            if agent.cu_per_token > query.max_cu_per_token {
                continue;
            }
            ...
            // Composite score.
            let tier_base = agent.tier.base_cu_per_token() as f64;
            let price_score = (1.0
                - agent.cu_per_token as f64 / (tier_base * Self::PRICE_SCORE_TIER_MULTIPLIER))
                .max(0.0);
            ...
            matches.push(CapabilityMatch {
                ...
                reputation: rep,
                price_score,
                cu_per_token: agent.cu_per_token,
                matched_model,
            });
        }
...
mod tests {
    ...
    fn test_composite_score_calculation() {
        let matcher = CapabilityMatcher;
        // Small tier base = 1 CU/token. cu_per_token=1, multiplier=4 → price_score = 1 - 1/4 = 0.75
