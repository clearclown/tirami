...
//! `ModelTier` is re-exported from `forge_ledger::lending` — not redefined here.

use serde::{Deserialize, Serialize};

use crate::errors::AgoraError;

// Re-export so downstream users can access ModelTier via forge_agora.
pub use forge_ledger::lending::ModelTier;
...
impl AgentProfile {
    pub fn new(
        ...
        if cu_per_token == 0 {
            return Err(AgoraError::InvalidTrade(
                "cu_per_token must be >= 1".to_string(),
            ));
        }
        Ok(Self {
            agent_hex,
...
mod tests {
    ...
    fn test_capability_query_default() {
        ...
        assert_eq!(q.max_cu_per_token, u64::MAX);
