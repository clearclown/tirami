...
//!
//! Ported from the Python scaffold `forge_bank/portfolio.py`.
...
impl PortfolioManager {
    ...
    pub fn apply(&mut self, decision: &Decision) {
        match decision.action {
            ...
            ActionKind::Lend => {
                if self.portfolio.cash_cu < decision.cu_amount {
                    return; // silently no-op
                }
                self.portfolio.cash_cu -= decision.cu_amount;
                self.portfolio.positions.push(Position::simple(PositionKind::Lent, decision.cu_amount));
            }

            ActionKind::Borrow => {
                self.portfolio.cash_cu += decision.cu_amount;
                self.portfolio.positions.push(Position::simple(PositionKind::Borrowed, decision.cu_amount));
            }
            ...
            ActionKind::Repay => {
                ...
                if let Some(idx) = self
                    ...
                    .positions
                    .iter()
                    .position(|p| p.kind == PositionKind::Borrowed && p.cu_amount == decision.cu_amount)
                {
                    self.portfolio.cash_cu -= decision.cu_amount;
    ...
    pub fn stats(&self) -> std::collections::HashMap<String, f64> {
        ...
        m.insert("cash_cu".into(), self.portfolio.cash_cu as f64);
...
mod tests {
    ...
    fn test_lend_decision_reduces_cash_and_creates_position() {
        ...
        assert_eq!(mgr.portfolio.cash_cu, 5_000);
    }
    ...
    fn test_borrow_decision_increases_cash_and_creates_position() {
        ...
        mgr.apply(&decision);
        assert_eq!(mgr.portfolio.cash_cu, 3_000);
    ...
    fn test_hold_does_nothing() {
        ...
        mgr.apply(&decision);
        assert_eq!(mgr.portfolio.cash_cu, 10_000);
    ...
    fn test_lend_silently_no_op_if_insufficient_cash() {
        ...
        mgr.apply(&decision);
        assert_eq!(mgr.portfolio.cash_cu, 100);
    ...
    fn test_stats_reflect_state() {
        ...
        let stats = mgr.stats();
        assert_eq!(stats["cash_cu"] as u64, 5_000);
