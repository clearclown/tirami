...
//!
//! Ported from the Python scaffold `forge_bank/types.py`.
...
///
/// Mirrors the JSON shape returned by `GET /v1/forge/pool`.
...
impl PoolSnapshot {
    ...
    pub fn utilization(&self) -> f64 {
        if self.total_cu == 0 {
            ...
            self.lent_cu as f64 / self.total_cu as f64
        }
    }
}
...
impl Position {
    /// Create a new position, validating that `cu_amount > 0`.
    pub fn new(kind: PositionKind, cu_amount: u64) -> Result<Self, BankError> {
        if cu_amount == 0 {
            return Err(BankError::InvalidParameter(
                "position cu_amount must be > 0".into(),
            ));
        }
        Ok(Self {
            kind,
    ...
    pub(crate) fn simple(kind: PositionKind, cu_amount: u64) -> Self {
        Self::new(kind, cu_amount).expect("cu_amount must be > 0")
    }
}
...
impl Portfolio {
    ...
    pub fn net_cu_exposure(&self) -> i64 {
        let positive = self.cash_cu as i64 + self.total_lent() as i64 + self.total_collateral() as i64;
...
impl Decision {
    /// Create a Decision, validating confidence and cu_amount rules.
    pub fn new(
        action: ActionKind,
        cu_amount: u64,
        ...
        if action != ActionKind::Hold && cu_amount == 0 {
            return Err(BankError::InvalidParameter(format!(
                "non-HOLD decision must have positive cu_amount, got {cu_amount}"
            )));
        }
        Ok(Self {
            action,
    ...
    pub(crate) fn make(
        ...
        Self::new(action, cu_amount, rationale, confidence).expect("Decision::make invalid args")
    }
}
