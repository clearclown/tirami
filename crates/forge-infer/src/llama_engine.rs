        ...
        loop {
            match token_rx.recv() {
                ...
                Ok(StreamEvent::Error(msg)) => {
                    return Err(ForgeError::InferenceError(msg));
                }
                Err(_) => {
...
fn run_generate(
    ...
    top_p: Option<f64>,
    top_k: Option<i32>,
) -> Result<Vec<(i32, Vec<u8>)>, ForgeError> {
    ...
    let be = backend.ok_or_else(|| ForgeError::InferenceError("not loaded".to_string()))?;
    let m = model.ok_or_else(|| ForgeError::InferenceError("not loaded".to_string()))?;
...
fn run_generate_streaming(
    ...
    let inner = move || -> Result<(), ForgeError> {
        let be = backend.ok_or_else(|| ForgeError::InferenceError("not loaded".to_string()))?;
        let m = model.ok_or_else(|| ForgeError::InferenceError("not loaded".to_string()))?;
