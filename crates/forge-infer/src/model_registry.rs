...
pub fn resolve(input: &str) -> Result<UnifiedResolvedModel, ForgeError> {
    ...
    Err(ForgeError::ModelLoadError(format!(
...
fn download_hf_file(
    ...
    let api = ApiBuilder::new()
        .with_progress(true)
        .build()
        .map_err(|e| ForgeError::ModelLoadError(format!("hf-hub init: {e}")))?;

    let model_repo = api.model(repo.to_string());
    let model_path = model_repo
        .get(file)
        .map_err(|e| ForgeError::ModelLoadError(format!("download {repo}/{file}: {e}")))?;
        ...
        Err(_) => {
            ...
            if non_gguf_repo != repo {
                ...
                alt_repo.get("tokenizer.json").map_err(|e| {
                    ForgeError::ModelLoadError(format!("download tokenizer: {e}"))
                })?
            } else {
