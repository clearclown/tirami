...
impl LoanRecord {
    ...
    ///
    /// Field order: lender(32) + borrower(32) + principal_cu(8) +
    /// interest_rate_per_hour(8, IEEE754 BE) + term_hours(8) +
    /// collateral_cu(8) + created_at(8) + due_at(8).
    ...
    pub fn total_interest(&self) -> u64 {
        (self.principal_cu as f64 * self.interest_rate_per_hour * self.term_hours as f64) as u64
    }

    /// Total amount borrower owes to clear the loan.
    pub fn total_due(&self) -> u64 {
        self.principal_cu.saturating_add(self.total_interest())
    }
}
...
pub fn trade_score_from_volume(total_cu: u64) -> f64 {
    (total_cu as f64 / TRADE_SCORE_CAP_CU as f64).min(1.0)
}
...
impl ModelTier {
    ...
    pub fn base_cu_per_token(self) -> u64 {
        match self {
...
mod tests {
    ...
    fn model_tier_base_prices_match_spec() {
        assert_eq!(ModelTier::Small.base_cu_per_token(), 1);
        assert_eq!(ModelTier::Medium.base_cu_per_token(), 3);
        assert_eq!(ModelTier::Large.base_cu_per_token(), 8);
        assert_eq!(ModelTier::Frontier.base_cu_per_token(), 20);
    }
