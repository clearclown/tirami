...
impl ForgeMindAgent {
    ...
    pub fn total_cu_invested(&self) -> u64 {
        self.history
            .iter()
            .map(|c| {
                c.proposal.cu_cost_to_propose
...
mod tests {
    ...
    async fn test_agent_keeps_improving_proposal() {
        ...
        let mut agent = ForgeMindAgent::new(
            h,
            Box::new(bench),
            Box::new(opt),
            Some(lenient_budget()),
