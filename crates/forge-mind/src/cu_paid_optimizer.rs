//! CuPaidOptimizer — a MetaOptimizer that calls a frontier LLM via reqwest
...
pub struct CuPaidOptimizer {
    ...
    /// CU per output token, used to compute cu_cost_to_propose from
    /// the response's token count. Frontier tier defaults to ~20 CU/token
    /// per forge-economics §2.
    pub cu_per_output_token: u64,
}

impl CuPaidOptimizer {
    pub fn new(
        ...
        Self {
            ...
            cu_per_output_token: 20,        // Frontier tier
        }
    }
}

#[async_trait]
impl MetaOptimizer for CuPaidOptimizer {
    fn name(&self) -> &str {
        "cu-paid-frontier"
    }
    ...
    async fn propose(&self, current: &Harness, baseline: &BenchmarkResult) -> ImprovementProposal {
        ...
        let cu_cost = output_tokens.saturating_mul(self.cu_per_output_token);
...
mod tests {
    ...
    async fn cu_paid_optimizer_fallback_on_network_error() {
        ...
        let opt = CuPaidOptimizer::new("http://127.0.0.1:1", "fake-key", "claude-sonnet-4-6");
        ...
        // Fallback path: cu_cost is 0 and rationale mentions "fallback"
    ...
    #[test]
    fn cu_paid_optimizer_estimated_cu_cost() {
        let opt = CuPaidOptimizer::new("http://example.com", "key", "model");
        assert_eq!(opt.estimated_cu_cost(), 10_000);
