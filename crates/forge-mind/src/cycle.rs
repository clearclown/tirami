...
impl ImprovementCycleRunner {
    ...
    pub fn run_one(&mut self, harness: &Harness, now_ms: u64) -> ImprovementCycle {
        ...
        if proposal.cu_cost_to_propose > 0 {
            if !self.budget.can_spend(proposal.cu_cost_to_propose) {
            ...
            let _ = self.budget.record_spend(proposal.cu_cost_to_propose);
        }
    ...
    pub fn budget(&self) -> &CuBudget {
        &self.budget
    }

    /// Access the budget (mutable).
    pub fn budget_mut(&mut self) -> &mut CuBudget {
        &mut self.budget
    }
...
mod tests {
    ...
    fn make_runner(
        ...
        cu_cost_per_benchmark: u64,
    ) -> ImprovementCycleRunner {
        let benchmark = InMemoryBenchmark::new("test", scoring_fn, 100, cu_cost_per_benchmark);
        let budget = CuBudget {
