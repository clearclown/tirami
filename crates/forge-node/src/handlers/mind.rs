//! HTTP handlers for `/v1/forge/mind/*` endpoints (Phase 8 / Batch B3).
//!
...
use forge_mind::{
    CuPaidOptimizer, EchoMetaOptimizer, ForgeMindAgent, Harness, InMemoryBenchmark,
...
pub struct MindBudgetRequest {
    #[serde(default)]
    pub max_cu_per_cycle: Option<u64>,
    #[serde(default)]
    pub max_cu_per_day: Option<u64>,
    #[serde(default)]
...
impl From<MindStats> for MindStatsResponse {
    fn from(s: MindStats) -> Self {
        Self {
            ...
            reverted: s.reverted,
            deferred: s.deferred,
            total_cu_invested: s.total_cu_invested,
...
pub(crate) async fn mind_init(
    ...
    let optimizer: Box<dyn forge_mind::MetaOptimizer> = match req.optimizer.as_str() {
        ...
        "cu_paid" => {
            ...
            Box::new(CuPaidOptimizer::new(api_url, api_key, model))
        }
        other => {
            return Err((
...
pub(crate) async fn mind_improve(
    ...
    for cycle in &cycles {
        let cu = cycle.proposal.cu_cost_to_propose;
        if cu > 0 {
...
pub(crate) async fn mind_budget(
    ...
    if let Some(v) = req.max_cu_per_cycle {
        budget.max_cu_per_cycle = v;
    }
    if let Some(v) = req.max_cu_per_day {
        budget.max_cu_per_day = v;
    }
    ...
    Ok(Json(MindBudgetResponse {
        ok: true,
        max_cu_per_cycle: budget.max_cu_per_cycle,
        max_cu_per_day: budget.max_cu_per_day,
        max_cycles_per_day: budget.max_cycles_per_day,
    }))
}

/// GET /v1/forge/mind/stats
