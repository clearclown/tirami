//! JSON persistence for L2/L3/L4 state (BankServices, Marketplace, ForgeMindAgent).
//!
...
use forge_agora::{AgentRegistry, Marketplace};
use forge_mind::{ForgeMindAgent, MindAgentSnapshot};
...
pub fn load_marketplace(path: &Path) -> io::Result<Option<Marketplace>> {
    ...
    let raw = fs::read_to_string(path)?;
    let snap: forge_agora::registry::RegistrySnapshot = serde_json::from_str(&raw)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
    let registry = AgentRegistry::restore(snap);
    let mp = Marketplace {
        registry,
        calculator: forge_agora::ReputationCalculator,
        matcher: forge_agora::CapabilityMatcher,
    };
    Ok(Some(mp))
}

// ---------------------------------------------------------------------------
// ForgeMindAgent snapshot
// ---------------------------------------------------------------------------

/// Persist a `MindAgentSnapshot` extracted from a live `ForgeMindAgent`.
///
/// The optimizer and benchmark are NOT included in the snapshot.
pub fn save_mind(agent: &ForgeMindAgent, path: &Path) -> io::Result<()> {
...
/// `ForgeMindAgent::restore_from_snapshot()`.
...
mod tests {
    use super::*;
    use forge_agora::types::{AgentProfile, ModelTier};
    use forge_mind::{EchoMetaOptimizer, Harness, InMemoryBenchmark};
    ...
    fn test_bank_round_trip() {
        ...
        assert_eq!(
            loaded.portfolio.portfolio.cash_cu,
            services.portfolio.portfolio.cash_cu
        );
    ...
    fn test_marketplace_round_trip() {
        ...
        assert_eq!(agent_a.cu_per_token, 3);
    ...
    fn test_mind_snapshot_round_trip() {
        ...
        assert_eq!(snap.budget.max_cu_per_cycle, agent.runner_budget().max_cu_per_cycle);
        ...
        let mut agent2 = ForgeMindAgent::new(harness2, Box::new(bench2), Box::new(opt2), None);
