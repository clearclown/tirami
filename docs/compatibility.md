...
> **TL;DR** — Forge is mesh-llm + a CU economic layer. Any GGUF model that runs on
> llama.cpp runs on Forge. Any OpenAI-compatible client that talks to mesh-llm
> talks to Forge. The only addition is `/v1/forge/*` (45 endpoints, all opt-in).
>
...
**Forge = mesh-llm runtime + Compute Unit economy.**
...
| OpenAI API | mesh-llm-derived axum router | `x_forge` extension on responses + 45 `/v1/forge/*` endpoints |
...
| Economy | (none in mesh-llm) | **everything** — CU ledger, lending, futures, RiskModel, forge-mind, forge-agora |
...
- **Architectures**: Llama 1/2/3/4, Qwen 2/2.5, Mistral, Mixtral (MoE),
  ...
  llama.cpp adds — there's no architecture-specific code in forge-infer.
...
[`crates/forge-infer/src/model_registry.rs`](../crates/forge-infer/src/model_registry.rs).
PRs welcome.
...
- ❌ `function_call` (deprecated, use `tools`), `n > 1` (single completion only),
  ...
  agora is for, see `/v1/forge/agora/*`)
...
| **PortfolioManager + futures + insurance** | ❌ | ❌ | ✅ (forge-bank) |
| **Self-improvement loop paid in CU** | ❌ | ❌ | ✅ (forge-mind) |
| **Reputation gossip + collusion detection** | ❌ | ❌ | ✅ (forge-agora) |
...
compatible inference server: ignore `/v1/forge/*`, leave `/metrics` unscraped,
...
# CU accounting starts immediately. Welcome loan = 1,000 CU at 0% interest.
```
...
Metal GPU) with no mock components. Every CU recorded in this transcript is
    ...
    -d '{"model":"smollm2:135m","messages":[{"role":"user","content":"What is 2+2?"}],"max_tokens":20}'
{
  ...
  "x_forge":{"cu_cost":21,"effective_balance":1021}
                                          ...
                                          ↑ real CU charged for 21 real tokens

$ curl localhost:3001/v1/forge/balance -H "Authorization: Bearer test-real-run"
...
$ curl 'localhost:3001/v1/forge/anchor?network=testnet' -H "Authorization: Bearer test-real-run"
{
...
Same node also responds to `/v1/forge/bank/tick`, `/v1/forge/agora/find`,
`/v1/forge/mind/improve`, `/v1/forge/credit`, `/v1/forge/pool`, etc — every
...
optionally bridge CU to BTC via Lightning if you need to.
...
laptop, Ollama is great. If you want a fleet of nodes that earn CU when
they're idle and spend CU when they need a bigger model, use Forge.
...
```
                ...
                └──────────┬────────────┘
                           ...
                           ▼
┌─────────────────────────────────────────────┐
│  L4 forge-agora    discovery + reputation   │
│  L3 forge-mind     self-improvement loop    │  ← clearclown/forge (this repo)
│  L2 forge-bank     finance instruments      │     5-layer Rust workspace
│  L1 forge-ledger   CU + lending + safety    │     12 crates
│  L0 mesh-llm       distributed inference    │  ← inherited from upstream
└─────────────────────────────────────────────┘
                  ▲                ▲
                  │                │
          ┌───────┴──┐      ┌──────┴────────┐
          │ forge-sdk│      │ forge-cu-mcp  │  ← Python clients for everything above
          │ (PyPI)   │      │ (PyPI + MCP)  │
          └──────────┘      └───────────────┘
...
| Earn CU by serving inference | `forge seed -m qwen2.5:1.5b` |
| Spend CU calling another node | `forge worker --seed <pubkey>` |
| Drive forge-bank from Python | `pip install forge-sdk==0.3.0` |
