...
I've been building **Forge** — a Rust protocol where computation is the medium of exchange. No token, no ICO, no speculation. 1 Compute Unit (CU) = 10^10 FLOPs of verified inference. You earn CU by running a local model for someone else; you spend CU by asking a node to run inference for you.
...
- **L3** — ForgeMindAgent: AI agent that self-improves by paying CU to a frontier model
...
**Verified end-to-end today** (2026-04-09): one command downloads SmolLM2-135M, starts a node, runs 3 real chat completions on Metal GPU, and you can watch real CU land in `/v1/forge/balance` while `/v1/forge/bank/tick` produces a lending Decision off the live pool state:
...
No token. No ICO. 1 CU = 10^10 FLOPs of verified inference.
...
You earn CU by running inference for others. You spend CU by asking someone else's node to run inference. That's the whole economy.

**3/12** 5-layer architecture, all Rust, single binary:

L0 — mesh-llm inference (Metal/CUDA/GGUF)
L1 — dual-signed CU ledger + lending pool
L2 — futures, insurance, VaR RiskModel
L3 — ForgeMindAgent (self-improves paid in CU)
L4 — Reputation marketplace

**4/12** The L3 piece is the most fun: `ForgeMindAgent::improve()` runs a self-optimization loop where a frontier model (Claude, GPT, local CuPaidOptimizer) proposes a new system prompt. If benchmark improves + ROI ≥ 1.0, commit. The API call cost is deducted from the agent's CU balance FOR REAL.
...
Plus you get `/v1/forge/{balance,trades,pool,bank,mind,agora,anchor,...}` — 45 economic endpoints.

**6/12** Phase 10 closeout metrics:

• 1,048 passing tests (forge 359 + forge-mesh 646 + forge-sdk 27 + forge-economics 16)
...
**9/12** Phase 10 P6 is wild: a `/v1/forge/anchor?network=mainnet` endpoint returns a Bitcoin OP_RETURN script carrying the 32-byte trade log Merkle root. You can broadcast it through any Bitcoin wallet and get a free integrity witness for your entire trade history.
...
Forge is literally mesh-llm + llama.cpp + a CU ledger + 4 layers of economic primitives stacked on top. You run `forge node -m smollm2:135m` and you get:

1. An OpenAI-compatible HTTP API on `http://localhost:3000/v1` (same shape as llama-server)
2. A Compute Unit ledger that records 1 CU per ~1 token of output
3. A P2P mesh (iroh QUIC + Noise) where your node can earn CU serving other people's requests
4. A lending pool, portfolio manager, futures contracts, insurance, reputation consensus
5. An AI self-improvement loop (ForgeMindAgent) that spends CU to improve its own prompts via frontier LLMs
...
- **Earn**: if you expose your node on your LAN/WAN, other nodes can pay you CU for inference. CU is earnable-only; no token, no ICO, no speculation.
- **Budget-aware agents**: the Python SDK `pip install forge-sdk` and MCP server `pip install forge-cu-mcp` let Claude Code / Cursor drive the full stack directly.
...
- `/v1/forge/mind/improve` with `CuPaidOptimizer` calls the Anthropic Messages API format only — GPT / Together / Groq are TODO.
...
A: No — Bittensor inserts the TAO token between validators and miners. Forge doesn't have a token. The unit of account IS the FLOP. You earn CU directly by serving inference; no validator pool, no yield farming, no speculation.

**Q: "How do you prevent Sybil attacks?"**
A: Welcome loan has a `welcome_loan_sybil_threshold: 100` — once you've seen >100 unknown nodes in a time window, further welcome-loan grants are denied. Plus `forge_ledger::collusion::CollusionDetector` runs Tarjan SCC on the trade graph to detect round-robin wash trading.
...
**Q: "How does the node earn CU when it's idle?"**
A: Two ways: (1) passive "availability yield" at 0.1%/hr × reputation (Phase 1), and (2) respond to inference requests from peers (the main mechanism). There's also a welcome loan of 1,000 CU at 0% interest for 72 hours so new nodes can experiment before needing to earn.

**Q: "What if I just want llama-server with no economy?"**
A: Disable the economic layer entirely — ignore `/v1/forge/*` and forge degrades to "llama-server with extra Rust crates compiled in". The CU accounting still runs, but if no one looks at `/v1/forge/balance`, it's harmless.

**Q: "Is this financialized? Does it leak compute into speculation?"**
A: No. CU is earnable-only — you literally cannot buy CU. There is no token, no ICO, no secondary market. Bridging to Bitcoin Lightning exists as an optional settlement path, but the core protocol has no fiat on-ramp in its design.
